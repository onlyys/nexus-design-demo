...
interface BlockRowProps {
  ...
  onAddBelow: () => void;
  onRichPaste?: (blockId: string, payload: RichPastePayload) => void;
  /** Enter 在光标位置切分当前段：before 写回当前 block，after 作为新 block 插到下方 */
  onSplit?: (blockId: string, before: string, after: string) => void;
  /** 行首 Backspace（非空）：把当前 html 合并到上一个 text-like block 末尾 */
  onMergeWithPrev?: (blockId: string, currentHtml: string) => boolean;
  /** 行末 Delete（非空）：把下一个 text-like block 内容合并到当前末尾 */
  onMergeNextIntoCurrent?: (blockId: string, currentHtml: string) => boolean;
  /** 跨 block 光标移动 */
  onArrowUpOut?: (blockId: string) => void;
  onArrowDownOut?: (blockId: string) => void;
}
...
export function BlockRow({
  ...
  return (
    <div className="group relative flex items-start gap-0 py-0.5 rounded-md transition-colors">
      {/* Hover handles：左外侧浮动，不与正文重叠 */}
      <div
        className={cn(
          "absolute right-full top-1 flex items-center gap-0.5 pr-1.5",
          "opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity",
        )}
      >
        <button
          onClick={onAddBelow}
          className="h-6 w-6 inline-flex items-center justify-center rounded text-ink-400 hover:text-ink-700 hover:bg-ink-100"
          title="在下方插入"
        >
        ...
        <button
          onClick={onDelete}
          className="h-6 w-6 inline-flex items-center justify-center rounded text-ink-400 hover:text-rose-600 hover:bg-rose-50 cursor-pointer"
          title="删除该 Block"
        >
      ...
      <div className="flex-1 min-w-0">
        <BlockContent
          ...
          onFocus={onFocus}
          onSlash={onSlash}
          onRichPaste={onRichPaste}
          onSplit={onSplit}
          onMergeWithPrev={onMergeWithPrev}
          onMergeNextIntoCurrent={onMergeNextIntoCurrent}
          onArrowUpOut={onArrowUpOut}
          onArrowDownOut={onArrowDownOut}
        />
      </div>
    </div>
  );
...
function BlockContent({
  ...
  onArrowUpOut,
  onArrowDownOut,
}: Omit<BlockRowProps, "onAddBelow">) {
  const richPasteHandler = onRichPaste
    ? (payload: RichPastePayload) => {
        onRichPaste(block.id, payload);
        return true;
      }
    : undefined;

  // 文本类 block 的统一编辑回调（text/h1/h2/h3/quote 共用）
  const textEditableProps = {
    onEnterSplit: onSplit
      ? (before: string, after: string) => onSplit(block.id, before, after)
      : undefined,
    onBackspaceAtStart: onMergeWithPrev
      ? (currentHtml: string) => onMergeWithPrev(block.id, currentHtml)
      : undefined,
    onDeleteAtEnd: onMergeNextIntoCurrent
      ? (currentHtml: string) => onMergeNextIntoCurrent(block.id, currentHtml)
      : undefined,
    onArrowUpAtStart: onArrowUpOut ? () => onArrowUpOut(block.id) : undefined,
    onArrowDownAtEnd: onArrowDownOut ? () => onArrowDownOut(block.id) : undefined,
  ...
  switch (block.type) {
    case "text":
      return (
        <Editable
          ...
          placeholder={isFocused ? "输入正文，输入 / 调出菜单" : ""}
          className="text-[15px] leading-[1.85] text-ink-900 py-1"
          onEnter={onEnter}
          onBackspaceEmpty={onDelete}
          onSlash={onSlash}
          onFocus={onFocus}
          onRichPaste={richPasteHandler}
          {...textEditableProps}
        />
      );
    case "h1":
      return (
        <Editable
          ...
          onChange={(text) => onUpdate({ ...block, text })}
          placeholder="标题 1"
          className="text-[28px] leading-[1.4] font-bold tracking-tight text-ink-900 py-1.5"
          onEnter={onEnter}
          onBackspaceEmpty={onDelete}
          onFocus={onFocus}
          onSlash={onSlash}
    ...
    case "h2":
      return (
        <Editable
          ...
          onChange={(text) => onUpdate({ ...block, text })}
          placeholder="标题 2"
          className="text-[22px] leading-[1.4] font-semibold tracking-tight text-ink-900 py-1.5"
          onEnter={onEnter}
          onBackspaceEmpty={onDelete}
          onFocus={onFocus}
          onSlash={onSlash}
    ...
    case "h3":
      return (
        <Editable
          ...
          onChange={(text) => onUpdate({ ...block, text })}
          placeholder="标题 3"
          className="text-[18px] leading-[1.4] font-semibold tracking-tight text-ink-900 py-1.5"
          onEnter={onEnter}
          onBackspaceEmpty={onDelete}
          onFocus={onFocus}
          onSlash={onSlash}
    ...
    case "quote":
      return (
        <div className="border-l-[3px] border-brand-500/70 pl-4 py-1 bg-brand-50/40 rounded-r-md">
          <Editable
            value={block.text}
            onChange={(text) => onUpdate({ ...block, text })}
            placeholder="引用"
            className="text-[14.5px] leading-[1.85] text-ink-700 italic"
            onEnter={onEnter}
            onBackspaceEmpty={onDelete}
            onFocus={onFocus}
            onSlash={onSlash}
      ...
      return (
        <ul className="space-y-1 py-0.5">
          {block.items.map((it, i) => (
            <li
              key={i}
              className="flex items-start gap-2.5 text-[15px] leading-[1.85] text-ink-900"
            >
              <span className="select-none mt-[7px] shrink-0 text-ink-500 font-medium tabular-nums min-w-[14px]">
              ...
              <Editable
                ...
                onBackspaceEmpty={() => {
                  if (block.items.length === 1) onDelete();
                  else {
                    const items = [...block.items];
                    items.splice(i, 1);
                    onUpdate({ ...block, items });
    ...
    case "image":
      return (
        <ResizableImage
          block={block}
          onUpdate={onUpdate as (b: Block) => void}
          onDelete={onDelete}
          onFocus={onFocus}
        />
      );
    ...
    case "todo":
      return (
        <ul className="space-y-1.5 py-1">
          {block.items.map((it, i) => (
            <li key={it.id} className="flex items-start gap-2.5 group/todo">
              <button
                onClick={() => {
                  const items = [...block.items];
              ...
              <Editable
                ...
                placeholder="待办事项"
                className={cn(
                  "flex-1 text-[14.5px] leading-[1.85]",
                  it.done ? "text-ink-400 line-through" : "text-ink-900",
                )}
                onEnter={() => {
                  const items = [...block.items];
                  items.splice(i + 1, 0, { id: uid(), text: "", done: false });
                  onUpdate({ ...block, items });
                }}
                onBackspaceEmpty={() => {
                  if (block.items.length === 1) onDelete();
    ...
    case "code":
      return (
        <div className="my-2 rounded-xl border border-ink-200 bg-[#0f172a] overflow-hidden">
          ...
          <Editable
            ...
            placeholder="// 代码"
            className="px-4 py-3 font-mono text-[12.5px] leading-[1.7] text-emerald-200 whitespace-pre"
            multiline
            onFocus={onFocus}
          />
        </div>
            ...
            <div className="bg-white">
              ...
              <div className="flex items-center gap-2 px-4 py-2 border-b border-ink-100 bg-ink-50/80">
                ...
                <a
                  ...
                  rel="noreferrer"
                  title="在浏览器中打开"
                  className="h-6 w-6 rounded-md text-ink-400 hover:text-ink-700 hover:bg-ink-100 inline-flex items-center justify-center"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_200px] gap-8 px-8 py-8 max-w-[1080px] mx-auto">
                  <article className="min-w-0">
                    ...
                    <ul className="mt-2 space-y-1.5 text-[14px] text-ink-700 list-disc pl-5 leading-[1.95]">
                      ...
                      <li>
                        模板在 18 个县复用，平均开发周期从 6 周缩短至 3 天
                      </li>
                      <li>
                        新增「合作社数字台账」轻量工具，已服务 320 家农民合作社
                      </li>
                    </ul>
                    ...
                    <p className="mt-2 text-[14px] leading-[1.95] text-ink-700">
                      与 27 所高校共建「数字乡村青年人才项目」，
                      累计培训 1.5 万人次，其中 62% 学员选择留在县域工作，远高于行业平均 31% 的留存率。
                    </p>
                    ...
                    <div className="mt-5 overflow-x-auto rounded-xl border border-ink-200">
                      <table className="min-w-full text-[13px]">
                        <thead className="bg-ink-50 text-ink-700">
                          <tr>
                            <th className="px-3.5 py-2 text-left font-semibold">
                              省份
                            </th>
                            <th className="px-3.5 py-2 text-left font-semibold">
                              覆盖县域
                    ...
                    <ul className="mt-2 space-y-1.5 text-[14px] text-ink-700 list-disc pl-5 leading-[1.95]">
                      <li>
                        与高校共同培养乡村振兴方向的专业人才，建立长期实习与就业通道
                      </li>
                      <li>
                        把数字工具箱以 SaaS 形式开放给县域政府与社会组织，免费使用
                      </li>
                      <li>
                        建立「为村开放联盟」，邀请更多企业、NGO、研究机构加入
                      </li>
                    </ul>
                    ...
                    {/* 作者 / 标签 / 评论 */}
                    <div className="mt-10 pt-5 border-t border-ink-100">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-gradient-to-br from-brand-500 to-violet-500 text-white font-bold flex items-center justify-center">
                          S
                        </div>
                        <div className="min-w-0">
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {[
                          ...
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 text-[11.5px] font-medium"
                          >
                            {t}
                          </span>
                        ))}
                    ...
                    <div className="mt-8 pt-5 border-t border-ink-100">
                      ...
                      <CommentItem
                        name="李建国"
                        title="云南元阳县乡村振兴办"
                        text="模板真的很好用！希望明年能多覆盖一些边远县。"
                      />
                      <CommentItem
                        name="马巍"
                        title="腾讯 SSV · 工程师"
                        text="后续我们会把 OpenAPI 开放给所有联盟伙伴。"
                      />
                      <CommentItem
                        name="王志恒"
                        title="腾讯 SSV · 项目经理"
                        text="欢迎更多有兴趣的同事加入「为村开放联盟」共建。"
                      />
                    </div>
...
function CommentItem({
  ...
  return (
    <div className="py-2.5 border-t border-ink-100 first:border-t-0">
      <div className="flex items-center gap-2">
        <div className="h-6 w-6 rounded-full bg-ink-200 text-ink-700 text-[11px] font-semibold flex items-center justify-center">
          {name.slice(0, 1)}
        </div>
        <div className="text-[12.5px] font-medium text-ink-900">{name}</div>
        <span className="text-[11px] text-ink-400">{title}</span>
      </div>
      <div className="mt-1 ml-8 text-[12.5px] text-ink-700 leading-relaxed">
        {text}
      </div>
    </div>
  );
